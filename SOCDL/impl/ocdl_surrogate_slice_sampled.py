# -*- coding: utf-8 -*-
"""Online version of slice based CDL with FISTA solver with sampled
stripe slices."""
import copy
import collections
import logging
...
class OnlineDictLearnSampledSliceSurrogate(
    ...
    class Options(cdict.ConstrainedDict):
        defaults = {
            ...
            'OCDL': {
                ...
                'NumSlices': -1,  # Numer of stripe slices to sample to start with
                'DiminishingSliceIter': -1  # If > 0, then take this iters to increase to total size
            }
        }
    ...
    def __new__(cls, *args, **kwargs):
        instance = super(OnlineDictLearnSampledSliceSurrogate, cls).__new__(cls)
    ...
    def __init__(self, D0, S0, lmbda=None, opt=None, dimK=1, dimN=2):
        ...
        """
        if opt is None:
            opt = OnlineDictLearnSampledSliceSurrogate.Options()
        assert isinstance(opt, OnlineDictLearnSampledSliceSurrogate.Options)
        self.opt = opt
    ...
    def solve(self, S, W=None):
        ...
        X_s, patches_s, gamma_s = self.sample_slices(X, patches)
        self.update_At_sampled(X_s, gamma_s)
        self.update_Bt_sampled(X_s, patches_s)
    ...
    def sample_slices(self, X, patches):
        """Sample slices and patches at the same location.
        ...
        -------
            X_s: [num_slices, M]
            gamma_s: [num_slices, 2*Hc-1, 2*Wc-1, M]
            patches_s: [num_slices, Hc, Wc, C]
        """
        # recap:
        # X has size
        #   [H, W, 1, 1, 1, K, M], where M is the number of atoms
        # patches has size
        #   [H, W, Hc, Wc, C, K, 1], where C is the channel dimension
        H, W, K, M = X.shape[0], X.shape[1], X.shape[-2], X.shape[-1]
        Hc, Wc, C = patches.shape[2:5]
        indices = np.random.choice(H*W*K, self.num_slices*K, replace=False)
        X_s = X.reshape(-1, X.shape[-1])[indices, ...]
        patches_s = np.moveaxis(patches.squeeze(-1), 5, 2)  # (H, W, K, Hc, Wc, C)
        patches_s = patches_s.reshape(-1, Hc, Wc, C)[indices, ...]

        gamma_s = np.zeros((len(indices), 2*Hc-1, 2*Wc-1, M), dtype=X.dtype)
        pad = [(Hc - 1, Hc - 1), (Wc - 1, Wc - 1)] + [(0, 0) for _ in range(X.ndim - 2)]
        Xp = np.pad(X, pad, 'wrap')  # (H+2*Hc-2, W+2*Wc-2, 1, 1, 1, K, M)
        axis_H, axis_W, axis_K = np.unravel_index(indices, [H, W, K])
        for cnt, (h, w, k) in enumerate(zip(axis_H, axis_W, axis_K)):
            gamma = Xp[h:h+2*Hc-1, w:w+2*Wc-1, 0, 0, 0, k, :]
            gamma_s[cnt] = gamma
        return X_s, patches_s, gamma_s

    def update_At_sampled(self, X_s, gamma_s):
        r"""Update At with sampled slices:
        ...
        """
        Lmbda_new = self.dtype.type(self.alpha * self.Lmbda + 1)
        gamma_s = gamma_s.reshape(gamma_s.shape[0], -1)
        new = gamma_s.T.dot(X_s)  # [2*Hc-1 x 2*Wc-1 x M, M]
        Hc, Wc = self.cri.shpD[:2]
        shp = [2*Hc-1, 2*Wc-1, 1, 1, 1, self.cri.M, self.cri.M]
        new = new.reshape(shp)
        self.At = np.asarray(
    ...
    def update_Bt_sampled(self, X_s, patches_s):
        r"""Update Bt with sampled slices:
        ...
        """
        Lmbda_new = self.dtype.type(self.alpha * self.Lmbda + 1)
        patches_s = patches_s.reshape(patches_s.shape[0], -1)
        new = patches_s.T.dot(X_s)
        Hc, Wc = self.cri.shpD[:2]
        shp = [Hc, Wc, 1, 1, self.cri.Cd, 1, self.cri.M]
        new = new.reshape(shp)
        self.Bt = np.asarray(
    ...
    def num_slices(self):
        """Number of slices."""
        total_slices = np.prod(self.S0.shape[:2])
        min_slices = self.opt['OCDL', 'NumSlices']
        diter = self.opt['OCDL', 'DiminishingSliceIter']
        if min_slices < 0:
            return total_slices
        else:
            if diter < 0:
                ...
                n = (self.j * total_slices + (diter-self.j) * min_slices) / diter
                return min(int(n), total_slices)
